...
def augment_data(images, masks, save_path, augment=True):
    ...
    for x, y in tqdm(zip(images, masks), total=len(images)):
        ...
        """Преобразование в трехканальное изображение, бинарное: (0 0 0), (1 1 1)"""
        (thresh, im_bw) = cv2.threshold(y, 64, 255, cv2.THRESH_BINARY)
        y = im_bw / 255
        y = y.astype(np.uint8)
        """Выясним, какие уникальные значения есть у масок"""
        # print("Unique values in masks are:", np.unique(y))
...
if __name__ == "__main__":
    ...
    """ Load the dataset """
    data_path = "/mnt/sda2/its/dataset/MVD20/unet/mvd_512"
